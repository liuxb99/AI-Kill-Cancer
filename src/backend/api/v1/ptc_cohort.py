"""Explainable, outcome-blind PTC research-case cohort comparison.
...
score. Outcome fields are deliberately excluded from ranking and are aggregated
only after the cohort has been selected.
"""
...
SCORING_VERSION = "ptc-cohort-outcome-blind-v2"
...
# Sum = 100. Outcome variables are intentionally absent.
WEIGHTS = {
    "genes": 42.0,
    "protein_variants": 23.0,
...
def compare_cases(anchor: PTCResearchCaseModel, candidate: PTCResearchCaseModel) -> dict[str, Any]:
    """Compare only pre-outcome molecular and clinicopathologic characteristics."""
    ...
    return {
        ...
        # Outcome information is returned for post-match descriptive analysis only.
        "case_facts": {
...
async def similar_cases(
    ...
    return {
        ...
        "methodology": {
            ...
            "candidate_window": min(len(rows), 1000),
        },
        "weights": WEIGHTS,
        "matches": matches,
        ...
        "trace": [
            ...
            {"step": 2, "name": "exclude_outcome_fields_from_similarity", "records": len(rows)},
            {"step": 3, "name": "compute_explainable_outcome_blind_similarity", "records": len(compared)},
            {"step": 4, "name": "rank_and_limit_before_outcome_review", "records": len(matches)},
            {"step": 5, "name": "aggregate_post_match_outcomes", "records": len(matches)},
        ],
        "disclaimer": (
            "Research cohort navigation only. Matching is outcome-blind: vital status, follow-up, "
            "death timing, and recorded outcomes are excluded from ranking and shown only after "
            "cohort selection. Similarity is not prognosis, diagnosis, treatment efficacy, or "
