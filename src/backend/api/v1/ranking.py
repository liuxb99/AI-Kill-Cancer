...
from src.backend.services.drug_ranking_service import DrugRankingService
...
def _error_id(request: Request) -> str:
    """從 request 取出可追蹤的 error_id，缺省時產生新值。"""
    return (
...
async def rank_variant(
    ...
    try:
        ...
        # Persist — transaction 由 DrugRankingService 管理；失敗即拋出（不吞錯）
        run_id = uuid.uuid4()
        ranking_result.id = str(run_id)
        ranking_service = DrugRankingService(db)
        await ranking_service.persist_run(**ranking_result.model_dump())
        ...
        logger.exception("Failed to rank variant [error_id=%s]", error_id)
        raise HTTPException(
            status_code=500,
            detail={
...
async def rank_case(
    ...
    try:
        ...
        # REVIEW-PHASE3F0-R3-P0-01 模式：不洩漏 str(e)，
        # 對外回傳固定訊息 + 可追蹤 error_id；完整例外寫入 server log。
        logger.exception("Failed to rank case [error_id=%s]", error_id)
        raise HTTPException(
            status_code=500,
            detail={
                "error": "internal_error",
