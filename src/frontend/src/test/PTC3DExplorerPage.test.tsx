...
vi.mock('../components/PTCCell3D', () => ({ default: ({ selectedCase, onSelectGene }: any) => <div><span>cell-view:{selectedCase?.case_id || 'none'}</span><button onClick={() => onSelectGene('BRAF')}>cell-braf</button></div> }))
vi.mock('../components/PTCProtein3D', () => ({ default: ({ structure }: any) => <div>protein-view:{structure?.gene || 'none'}</div> }))
vi.mock('../components/PTCTargetingPanel', () => ({ default: ({ gene }: any) => <div>targeting-view:{gene || 'none'}</div> }))
vi.mock('../components/PTCLiteratureAssetsPanel', () => ({ default: ({ gene }: any) => <div>literature-view:{gene || 'none'}</div> }))

vi.mock('../api/ptcVisualization', () => ({
  getLatestPTCCases: vi.fn().mockResolvedValue({ count: 2, limit: 100, cases: [
    { case_id: 'TCGA-NEW-001', source_dataset: 'TCGA-THCA', source_project: 'TCGA-THCA', disease: 'papillary_thyroid_carcinoma', updated_at: '2026-08-01T02:00:00', pathologic_stage: 'Stage I', vital_status: 'Alive', variants: [{ variant_id: 'v1', gene: 'BRAF', protein_change: 'p.V600E' }], outcomes: [] },
    { case_id: 'TCGA-OLD-001', source_dataset: 'TCGA-THCA', source_project: 'TCGA-THCA', disease: 'papillary_thyroid_carcinoma', updated_at: '2026-07-31T02:00:00', pathologic_stage: 'Stage II', vital_status: 'Alive', variants: [{ variant_id: 'v2', gene: 'RET' }], outcomes: [] },
  ] }),
  getPTCProteinStructure: vi.fn().mockImplementation(async (gene: string) => ({ gene, name: `${gene} protein`, uniprot: 'P15056', alphafold_entry_id: 'AF-P15056-F1', alphafold_entry_url: 'https://alphafold.example/BRAF', cif_url: 'https://alphafold.example/BRAF.cif', pdb_url: 'https://alphafold.example/BRAF.pdb', pdb_urls: ['https://alphafold.example/BRAF.pdb'], experimental_structures: [], experimental_pdb_ids: ['1UWH'], default_pdb_id: '1UWH', renderer: 'builtin-threejs-pdb', uses_alphafold_api: false, source: 'test', disclaimer: 'test' })),
}))
...
describe('PTC3DExplorerPage', () => {
  it('lists the latest cases and switches the selected case', async () => {
    ...
    const recentCases = await screen.findByText('最近 100 個病例')
    const aside = recentCases.closest('aside')!
    expect(within(aside).getByText(/TCGA-NEW-001/)).toBeInTheDocument()
    expect(within(aside).getByText(/TCGA-OLD-001/)).toBeInTheDocument()
    expect(screen.getByText('cell-view:TCGA-NEW-001')).toBeInTheDocument()

    fireEvent.click(within(aside).getByText(/TCGA-OLD-001/))
  ...
  it('filters the latest cases by gene or case id', async () => {
    ...
    const aside = screen.getByText('最近 100 個病例').closest('aside')!

    expect(within(aside).queryByText(/TCGA-NEW-001/)).not.toBeInTheDocument()
    expect(within(aside).getByText(/TCGA-OLD-001/)).toBeInTheDocument()
  ...
  it('switches all four views and keeps the URL reproducible', async () => {
    render(<PTC3DExplorerPage />)
    await screen.findByRole('heading', { name: 'TCGA-NEW-001' })
