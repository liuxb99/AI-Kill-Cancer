...
vi.mock('../api/ptcResearch', () => ({
  getPTCCase: vi.fn().mockResolvedValue({
    case_id: 'TCGA-MATRIX-999',
    source_dataset: 'TCGA-THCA',
    source_project: 'TCGA-THCA',
    disease: 'papillary_thyroid_carcinoma',
    variants: [{ variant_id: 'v9', gene: 'RET', protein_change: 'fusion' }],
    outcomes: [],
  }),
}))
...
vi.mock('../api/ptcEvidenceMatrix', () => ({
  getPTCEvidenceMatrix: vi.fn().mockResolvedValue({
    ...
    methodology: {
      ...
      maximum_score: 100,
      weights: { variant_present: 20, persisted_therapies: 20, best_evidence_level: 30, active_trials: 15, open_full_text_assets: 10, source_provenance: 5 },
      outcome_blind: true,
      outcome_fields_excluded: ['vital_status', 'days_to_last_follow_up', 'days_to_death', 'outcomes'],
    ...
    rows: [{
      ...
      cohort: { role: 'post_score_descriptive_only', excluded_from_score: true, same_gene_cases: 12, vital_status_distribution: { Alive: 10, Dead: 2 }, outcome_distribution: {} },
      assets: { figures: 1, tables: 1 },
      gaps: [],
    }],
    summary: { genes: 1, therapies: 1, evidence: 1, trials: 1, open_full_text_assets: 2, unresolved_gaps: 0 },
    trace: [],
    disclaimer: 'Scores measure imported data linkage and provenance completeness.',
  }),
}))
