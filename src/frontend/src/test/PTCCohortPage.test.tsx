...
const { navigate, getPTCCase, getPTCSimilarCases } = vi.hoisted(() => ({
  navigate: vi.fn(),
  getPTCCase: vi.fn(),
  getPTCSimilarCases: vi.fn(),
}))
...
const response = {
  ...
  matches: [{
    ...
    components: { genes: 42, protein_variants: 23, pathologic_stage: 15, tnm: 10, age_range: 5, sex: 5 },
  ...
  disclaimer: 'Matching is outcome-blind.',
}

beforeEach(() => {
  navigate.mockReset()
  getPTCCase.mockReset()
  getPTCSimilarCases.mockReset()
...
describe('PTCCohortPage', () => {
  ...
  it('supports exact full-database case lookup in advanced mode', async () => {
    render(<MemoryRouter><PTCCohortPage /></MemoryRouter>)
    await screen.findByDisplayValue('TCGA-ANCHOR')
