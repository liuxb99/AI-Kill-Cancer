...
export default function ClinicalDecisionListPage() {
  ...
  async function selectAdvancedPatient(value: string) {
    ...
    try {
      ...
      setPatients((current) =>
        current.some((item) => item.patient_id === patient.patient_id)
          ? current
  ...
  const recentContent = (
    <div className="divide-y">
      {patients.length === 0 && !loading ? (
        <p className="p-6 text-center text-sm text-slate-500">目前沒有可供選擇的患者。</p>
      ) : (
        patients.map((patient) => (
          <button
            ...
            className={`block w-full px-4 py-3 text-left hover:bg-slate-50 ${
              patient.patient_id === patientId ? 'bg-indigo-50 text-indigo-700' : 'text-slate-700'
            }`}
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <header className="mb-6 flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-semibold text-primary-600">Clinical Decision</p>
          <h1 className="text-3xl font-bold">臨床決策列表</h1>
      ...
      <DualModeSelector
        title="患者選擇"
        description="最近 100 位患者快速選擇，或查詢完整資料庫中的 Patient ID。"
        recentContent={recentContent}
        advancedLabel="完整 Patient ID"
        advancedPlaceholder="輸入完整 UUID Patient ID"
        advancedValue={advancedValue}
        onAdvancedValueChange={setAdvancedValue}
        onAdvancedSubmit={() => selectAdvancedPatient(advancedValue)}
        advancedLoading={loading}
        advancedHelp="進階模式會先確認患者存在，再使用相同結果區載入臨床決策。"
      />
      ...
      {!loading && result && result.decisions.length > 0 && (
        <section className="mt-6 overflow-hidden rounded-xl border bg-white shadow-sm">
          <div className="flex items-center justify-between border-b px-5 py-4">
            <h2 className="font-semibold">{patientId} 的決策</h2>
            <span className="text-xs text-gray-400">最多展示 100 筆 · 共 {result.total ?? result.decisions.length} 筆</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-left text-xs uppercase text-gray-500">
                <tr>
                  <th className="px-5 py-3">決策類型</th>
                  <th className="px-5 py-3">信心等級</th>
              ...
              <tbody className="divide-y">
                {result.decisions.slice(0, 100).map((decision: ClinicalDecisionResponse) => (
                  <tr
                    key={decision.decision_id}
                    className="cursor-pointer hover:bg-gray-50"
                    onClick={() => navigate(`/clinical-decision/${decision.decision_id}`)}
                  >
                    <td className="px-5 py-4 font-medium">{decision.decision_type || '—'}</td>
                    <td className="px-5 py-4">
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs ${confidenceBadge(decision.confidence)}`}>
                        {decision.confidence || '—'}
                      </span>
                    </td>
                    <td className="px-5 py-4 font-mono text-xs">{decision.patient_id || '—'}</td>
                    <td className="px-5 py-4 text-gray-500">{decision.created_at ? formatDateTime(decision.created_at) : '—'}</td>
                    <td className="px-5 py-4 text-primary-600">查看詳情 →</td>
                  </tr>
                ))}
              </tbody>
            </table>
