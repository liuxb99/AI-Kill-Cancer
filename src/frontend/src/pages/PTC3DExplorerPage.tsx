...
export default function PTC3DExplorerPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setSelectedCase(result.cases[0] || null)
      } catch (reason) {
  ...
  async function selectGene(gene: string) {
    ...
    try {
      ...
      setProtein(null)
      setError(reason instanceof Error ? reason.message : `${gene} 尚无可用结构映射`)
    } finally {
            ...
            {!loading && cases.map((item, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="font-semibold text-gray-900">{index + 1}. {item.case_id}</span>
                  <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{item.variants.length} variants</span>
                </div>
                <div className="mt-1 text-xs text-gray-500">
                  {item.pathologic_stage || 'Stage 未提供'} · {item.vital_status || 'Outcome 未提供'}
                </div>
                <div className="mt-1 truncate text-[11px] text-gray-400">
                  {item.updated_at ? `更新 ${new Date(item.updated_at).toLocaleString()}` : item.source_dataset}
                </div>
              </button>
            ))}
            {!loading && cases.length === 0 && <div className="p-8 text-center text-gray-500">尚未下载病例，请先在 PTC 总控台执行同步。</div>}
          </div>
        </aside>
          ...
          <div className="rounded-xl border bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex gap-2">
                <button className={`rounded px-4 py-2 text-sm font-semibold ${view === 'cell' ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-700'}`} onClick={() => setView('cell')}>癌细胞 3D</button>
                <button className={`rounded px-4 py-2 text-sm font-semibold ${view === 'protein' ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-700'}`} onClick={() => setView('protein')}>蛋白折叠 3D</button>
              </div>
            </div>
