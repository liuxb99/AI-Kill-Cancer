...
def upgrade() -> None:
    ...
    # Recreate table to change constraints (required for SQLite compatibility)
    with op.batch_alter_table("domain_treatment_plans", recreate="always") as batch_op:
        # plan_id: remove unique, keep nullable=False, index=True
        # Must explicitly drop the old unique index created by unique=True in 023;
        # batch mode with recreate="always" does NOT automatically remove it.
        ...
        # Add FK constraints (self-referencing to domain_treatment_plans.id)
    ...
    with op.batch_alter_table("domain_treatment_plan_traces", recreate="always") as batch_op:
        # trace_id: remove unique, keep nullable=False, index=True
        # Must explicitly drop the old unique index created by unique=True in 023
...
def downgrade() -> None:
    # ─── domain_treatment_plans ─────────────────────────────────────────
    with op.batch_alter_table("domain_treatment_plans", recreate="always") as batch_op:
        # Drop FK constraints first (order matters in batch mode)
        ...
        # Drop version link columns (FK targets)
        batch_op.drop_column("supersedes_version_id")
        batch_op.drop_column("previous_version_id")
        # Drop the composite unique added by 025 upgrade
        batch_op.drop_constraint("uq_plan_id_version", type_="unique")
        # Restore single-column unique on plan_id (explicit create_index with
        # unique=True is needed because alter_column(unique=True) is unreliable
    ...
    with op.batch_alter_table("domain_treatment_plan_traces", recreate="always") as batch_op:
        ...
        # Restore single-column unique on trace_id
        batch_op.alter_column("trace_id", existing_type=sa.String(64), nullable=False)
        batch_op.create_index("ix_domain_treatment_plan_traces_trace_id", ["trace_id"], unique=True)
