...
import pytest
from src.backend.pipeline.normalization import normalize_variants_python, BcftoolsAdapter
...
class TestPythonNormalization:
    def test_basic_snv_no_normalization_needed(self):
        ...
        result = normalize_variants_python(variants)
    ...
    def test_left_alignment_with_common_prefix(self):
        ...
        assert len(result.normalized) == 1
        nv = result.normalized[0]
        # "A" is common prefix, trimmed to G>T
        assert nv.reference == "G"
        assert nv.alternate == "T"
    ...
    def test_empty_input(self):
        result = normalize_variants_python([])
